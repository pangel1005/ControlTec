// src/pages/upc/UpcDashboard.jsx
import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import api from "../../api/apiClient";

export default function UpcDashboard() {
  ...
  useEffect(() => {
    const cargarSolicitudes = async () => {
      ...
      try {
        // El backend ya filtra por rol: TecnicoUPC -> estados asignados
        const res = await api.get("/api/Solicitudes");
        const data = res.data || [];

        // Por seguridad puedes filtrar solo Validación Recepción
        const normalizar = (s = "") =>
          s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();

        const enValidacion = data.filter(
          (s) => normalizar(s.estado) === "validacion recepcion"
        );

        setSolicitudes(enValidacion);
      } catch (err) {
        console.error("Error cargando solicitudes UPC:", err);
        const status = err.response?.status;
        if (status === 401) {
          setError("Tu sesión ha expirado. Vuelve a iniciar sesión.");
        } else if (status === 403) {
          setError("No tienes permiso para ver estas solicitudes.");
        } else {
